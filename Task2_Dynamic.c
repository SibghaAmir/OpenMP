...
// OpenMP Dynamic Scheduling Transpose
void transposeMatrixDynamic(int matrix[ROWS][COLS], int transposed[COLS][ROWS], int num_threads) {
    omp_set_num_threads(num_threads);  // Set number of threads

    double start_time = omp_get_wtime();

    #pragma omp parallel for collapse(2) schedule(dynamic, BLOCK_SIZE)
    ...
    printf("Threads: %d, Scheduling: Dynamic, Execution Time: %f seconds\n", num_threads, end_time - start_time);
}
...
int main() {
    ...
    for (int i = 0; i < 3; i++) {
        transposeMatrixDynamic(matrix, transposed, thread_counts[i]);
    }
