...
double collapse_dynamic(double A[MAX_SIZE][MAX_SIZE], double B[MAX_SIZE][MAX_SIZE], int N, int chunk_size) {
...
int main() {
    ...
    int chunk_sizes[] = {128,250};  // Different chunk sizes
    ...
    for (int t = 0; t < 2; t++) {
        ...
        for (int c = 0; c < 2; c++) {
            ...
            for (int s = 0; s < 3; s++) {
                ...
                double exec_time = collapse_dynamic(A, B, N, chunk_size);
